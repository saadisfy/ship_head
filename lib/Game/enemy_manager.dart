...
class EnemyManager extends BaseComponent with GameSize, HasGameRef<GameWorld>{
  ...
  void _spawnEnemy() {
    ...
    position.clamp(Vector2.zero(), gameSize - initSize / 2); //clamp the randomness, so it will always spawn between the borders
    Enemy enemy = Enemy(
      sprite: spriteSheet.getSpriteById(2),
      size: initSize,
      position: position,
    );
    enemy.anchor = Anchor.center; //so that the starting position at the spawning doesn't go over the borders
    addChild(enemy);
  }
